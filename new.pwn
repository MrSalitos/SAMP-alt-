...
public OnPlayerCommandText(playerid, cmdtext[])
{
  ...
  else if(!strcmp(strget(cmdtext, 0), "/schock") && CopOnDuty[playerid] == true && playerdata[playerid][2] == 4)													// Macht das Ziel fьr 7 Sekunden bewegungsunfдhig
  {
		new Float:pX, Float:pY, Float:pZ;
		GetPlayerPos(strval(strget(cmdtext, 1)), pX, pY, pZ);
  ...
  else if(!strcmp(strget(cmdtext, 0), "/ergeben"))			// Nimmt die Hдnde hoch
  {
  ...
  else if(!strcmp(strget(cmdtext, 0), "/verhaften") && CopOnDuty[playerid] == true && playerdata[playerid][2] == 4)	// Bringt den Verdдchtigen ins Gefдngnis	{
  {
		new Float:pX, Float:pY, Float:pZ;
		GetPlayerPos(strval(strget(cmdtext, 1)), pX, pY, pZ);
...
public OnPlayerDisconnect(playerid, reason)
{
  ...
  KillTimer(toroeffner[playerid]); 		// Tьrtimer stoppen
...
public OnPlayerStateChange(playerid, PLAYER_STATE:newstate, PLAYER_STATE:oldstate)
{
  ...
  {
	//Zeige Tacho fьr alle Fahrzeuge ausser Fahrrдder
      ...
      if(showtacho(playerid))
	  {
		...
		for(new i = 0; i < sizeof(mietwagen); i++)
		{
		  if(GetPlayerVehicleID(playerid) == mietwagen[i][0])
		  {
			...
			{
			  new output[128];
			  format(output, sizeof(output), "*** Die Restliche Mietdauer betrдgt %i Minuten ***", floatround(mietwagen[i][10]));
		...
		if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 403 || GetVehicleModel(GetPlayerVehicleID(playerid)) == 514)
		{
		  ...
		  else
		  {
			  ...
			  SendClientMessage(playerid, COLOR_TRUCKER, "*** Nur Trucker dьrfen dieses Fahrzeug benutzen ***");
		  }
		}
		//----------Limofahrer-------------------
		...
		else if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 420)
		{
		  ...
		  else
		  {
			...
			GameTextForPlayer(playerid, string, 3000, 1);
			SendClientMessage(playerid, COLOR_TRUCKER, "*** Nur Taxifahrer dьrfen dieses Fahrzeug benutzen ***");
		  }
		}
		//---------- POLIZIST ----------------
		if(PLAYERCAR_IS_COP)
		{
				...
				else
				{
					...
					GameTextForPlayer(playerid, string, 3000, 1);
					SendClientMessage(playerid, COLOR_COP, "*** Nur Polizeibeamte dьrfen dieses Fahrzeug benutzen ***");
				}
		}
		if(PRODCAR_LS)
		{
		  ...
		  else
		  {
			...
			GameTextForPlayer(playerid, string, 3000, 1);
			SendClientMessage(playerid, COLOR_TRUCKER, "*** Nur Lieferanten dьrfen dieses Fahrzeug benutzen ***");
		  }
		}
		if(PRODCAR_SF)
		{
	...
	for(new i = 0; i < sizeof(trucker_car[]); i++)
	{
	  if(vehicleid == trucker_car[1][i])
	  {
		...
		if(vhealth == 0)
		{
		  ...
		  GameTextForPlayer(playerid, "~r~Mission felgeschlagen!~n~Ihr Fahrzeug wurde zerstцrt.", 3000, 3);
		}
	  }
	}
  }
